...
# from neck.yolo_fpn import YOLOXPAFPN
# from head.yolo_head import YOLOXHead
# from losses import YOLOXLoss
# from post_process import yolox_post_process
# from ops import fuse_model
...
# def get_model(opt, num_classes):
    ...
    # head = YOLOXHead(num_classes=num_classes, reid_dim=opt.reid_dim, width=width, in_channels=in_channel,
    #                  depthwise=opt.depth_wise)
...
if __name__ == "__main__":
    from thop import profile # used to compute the number of totla params and GFlops

    image = torch.rand([1, 3, 640, 640])
    # model = get_model(opt, num_classes = 80) #make sure the size of the model in opt is what you want
    model = get_model(opt)
    # yolo_outputs = model(image) # not pre-trained weights
    # print(yolo_outputs[0].shape)
    # print(yolo_outputs[1].shape)
    # print(yolo_outputs[2].shape)
    
    # pre-trained weights
    # path = '../yolox_coco_weights/yolox-tiny.pth'
    # path = '../yolox_coco_weights/yolox-nano.pth'
    # path = '../yolox_coco_weights/yolox-s.pth' #remember weights need to match the model size 
    path = '../yolox_coco_weights/yolox-m.pth'
    # path = '../yolox_coco_weights/yolox-l.pth'
    # path = '../yolox_coco_weights/yolox-x.pth'
    
    pre_trained_model = load_model(model, path) # from utils/model_utils
    
    yolo_outputs = pre_trained_model(image)
    print(yolo_outputs[0].shape)
    print(yolo_outputs[1].shape)
    print(yolo_outputs[2].shape)
    
    
    # total_ops, total_params = profile(model, (image,))
