...
# from backbone.csp_darknet import BaseConv, DWConv
...
if __name__ == "__main__":
    ...
    # feats = [torch.rand([1, in_channel[0], 64, 64]), 
    #          torch.rand([1, in_channel[1], 32, 32]),
    #          torch.rand([1, in_channel[2], 16, 16])]
    
    feats = [torch.rand([1, in_channel[0], 80, 80]), 
             torch.rand([1, in_channel[1], 40, 40]),
             torch.rand([1, in_channel[2], 20, 20])]
    
    
    head = YOLOXHead(num_classes=20, reid_dim=0) #reid_dim???
    # head.init_weights()
    head.eval()
